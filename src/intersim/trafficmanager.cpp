...
: Module( 0, "traffic_manager" )
{
   ...
   //UNT added
   _core_latency_stats = new Stats * [2];
   ...
   //UNT added
   for ( int i = 0; i < 2; ++i) {
      tmp_name<<"core_"<<i<<"_stat_";
      _core_latency_stats[i] = new Stats( this, tmp_name.str(), 1.0, 1000 );
      tmp_name.seekp( 0, ios::beg );
   }
   ...
   if ( f->tail || _flit_timing ) {
      ...
      if ( ( _sim_state == warming_up ) || f->record ) {
         if ( f->true_tail || _flit_timing ) {
            ...
            case none:
               _core_latency_stats[f->core]->AddSample( (_time - f->time) );
...
void TrafficManager::_GeneratePacket( int source, int psize /*# of flits*/ , 
                                      int cl, int time, void* data, int dest, int core )
{
   Flit *f;
   bool record;
   bool split_head;
...
bool TrafficManager::_SingleSim( )
{
   ...
   while ( ( total_phases < _max_samples ) && 
      ...
      //UNT added
      CPU_latency = _core_latency_stats[0]->Average( );
      GPU_latency = _core_latency_stats[1]->Average( );

      cout << "% Average latency = " << cur_latency << endl;

      //UNT added
      cout << "% CPU Average latency = " << CPU_latency << endl;
      cout << "% GPU Average latency = " << GPU_latency << endl;
