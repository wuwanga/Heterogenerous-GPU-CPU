...
void create_node_map(int n_shader, int n_mem, int n_cpu, int size, int use_map)
{
   ...
   if (use_map) {
	...
	// for CPU+GPU mesh topology 8 MC, 14 CPU, 14*2 GPU
            //int newmap[]  = {
            //   1, 3, 5, 6, 8, 10, 13, 15, 19, 21, 24, 26, 28, 35, //GPU
            //   0, 2, 4, 7, 9, 12, 14, 16, 18, 20, 22, 25, 27, 30, //CPU
            //   11, 17, 23, 29, 31, 32, 33, 34 //MC
            //};
...
{ 
   ...
   if (fixed_lat_icnt) {
      ...
      if (!doub_net && !quad_net) {
         nc=0;
         if ( is_cpu(input) || (is_mem(input) && is_cpu(output)) ) {
            core = 0;
         } else {
            core = 1;
         }

         traffic[nc]->_GeneratePacket( input, n_flits, 0 /*class*/, traffic[nc]->_time, data, output, core);
         ...
         if ( is_cpu(input) || (is_mem(input) && is_cpu(output)) ) {
          ...
          if (is_shd(input) && is_mem(output) ) {
         	 nc=0;
         	traffic[nc]->_GeneratePacket( input, n_flits_gpu, 0 /*class*/, traffic[nc]->_time, data, output, core);
                //printf("Generate packet GPU to mem: from %d to %d; %d flits\n", input, output, n_flits_gpu);
          } else if (is_cpu(input) && is_mem(output)) {
         	 nc=2;
         	traffic[nc]->_GeneratePacket( input, n_flits_cpu, 0 /*class*/, traffic[nc]->_time, data, output, core);
